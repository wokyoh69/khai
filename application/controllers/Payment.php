...
class Payment extends BaseController
{
    ...
    */
    ...
    /*Whatsapp*/
    public function whatsappDetail() 
    {
        $userid = $this->input->post('id'); //get userid from jquery
        ...
        if ($row) {
            ...
            //$this->global['pageTitle'] = 'Surat Peringatan'; /*tambah*/
            //$this->loadViews('payment/payment_warning', $this->global, $data, NULL); /*tambah*/
            $this->load->view('payment/send_reminder',$data); 
        } 
    ...
    if($this->isAdmin() == TRUE)
        ...
        if ($row) {
            $data = array(
                ...
                'billcode' => set_value('billcode', $row->pg_billcode),
                'transactionid' => set_value('transactionid', $row->pg_transactionid),
    ...
    public function update_action() 
    {
        ...
        if(!empty($this->input->post('fpx'))){
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(!empty($this->input->post('billcode',TRUE))){
                    if(empty($this->input->post('catatan',TRUE))) {
                        $catatan = "FPX";
                    } else {
                        $catatan = $this->input->post('catatan',TRUE);
                    }
                ...
                $data = array(
            		...
            		'catatan' => $catatan,
                    'pg_msg' => $pg_msg,
                    'pg_billcode' => $this->input->post('billcode',TRUE),
                    'pg_transactionid' => $this->input->post('transactionid',TRUE),
                    'updatedBy'=>$this->vendorId, //adminID
                    'updatedDtm'=>date('Y-m-d H:i:s'),
            	    );
    ...
    public function paymentDetail()
    {
        ...
        if ($row) {
            $data = array(
                ...
                'profile_url' => $info->g_profile_url,
                'role_user' => $this->role,
                );
